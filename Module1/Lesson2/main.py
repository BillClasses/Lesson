...
def chuoixuly(chuoi):
    ...
    for i in chuoi:
        result += 1
    print("Chuoi nay dai {} ki tu".format(result))
    
chuoixuly(chuoi)
print(xulychuoi(chuoi), "Ki tu")

# Tạo một danh sách các số nguyên. Viết một phương thức để đếm số lượng số lẻ trong danh sách.
lst = [123, 333, 55, 677, 123, 3434, 7887, 12321, 5324]
def demsole(lst):
    tongsole = 0
    for i in lst:
        if i % 2 != 0:
...
def xulydanhsachnhieuchuoi(lst):
    for i in lst:
        counter = 0
        for j in i:
            counter += 1
...
xulydanhsachnhieuchuoi(danhsachnhieuchuoi)
